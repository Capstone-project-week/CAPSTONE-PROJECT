...
export class CanvasComponent implements OnInit {
  ...
  regenerateImage() {
    ...
    let basePrompt = node.content || node.title || 'A beautiful story scene';
    let promptText = `A high quality storybook illustration of: ${basePrompt.substring(0, 150)}`;
    
    setTimeout(() => {
      const seed = Math.floor(Math.random() * 100000);
      node.image = `https://image.pollinations.ai/prompt/${encodeURIComponent(promptText)}?seed=${seed}&width=600&height=400`;
